...
core::MonitorPool* global_pool = nullptr;

void signal_handler(int) {
    running = 0;
    std::cout << "\n[SHUTDOWN] Stopping all monitors..." << std::endl;
    if (global_pool) {
        global_pool->stopAll();
    }
}
...
int cmd_run() {
    ...
    std::cout << "Starting multi-creator monitoring..." << std::endl;
    std::cout << "Creators: " << creators.size() << std::endl;
    ...
    for (const auto& name : creators) {
        pool.addCreator(name);
    }
    
    std::signal(SIGINT, signal_handler);
    std::signal(SIGTERM, signal_handler);
    
    std::cout << "Press Ctrl+C to stop all monitors." << std::endl;
    std::cout << std::endl;
    ...
    auto states = pool.getAllStates();
    int total_messages = 0;
    int total_bursts = 0;
    for (const auto& state : states) {
        total_messages += state.messages_processed;
        total_bursts += state.bursts_detected;
    }
